...
describe('MemoryAdapter', () => {
	...
	describe('setField()', () => {
		it('updates the persisted node to include the new field', async () => {
			...
			await adapter.setField(node, 'wizard', 'gandalf');
		...
		it('returns the new node', async () => {
			...
			const result = await adapter.setField(node, 'wizard', 'gandalf');
	...
	describe('addValueToSet()', () => {
		describe('when field does not exist on node', () => {
			it('raises an exception', async () => {
				...
				try {
					await adapter.addValueToSet(node, 'wizards', 'gandalf');
				} catch (error) {
					expect(error.message).toStrictEqual(
		...
		describe('when set already includes specified value', () => {
			...
			it('returns the same node', async () => {
				...
				const result = await adapter.addValueToSet(node, 'wizards', 'gandalf');
	...
	describe('removeValueFromSet()', () => {
		describe('when field does not exist on node', () => {
			it('raises an exception', async () => {
				...
				try {
					await adapter.removeValueFromSet(node, 'wizards', 'gandalf');
				} catch (error) {
					expect(error.message).toStrictEqual(
						'Unable to find field for key: wizards'
					);
	...
	describe('addValueToList()', () => {
		describe('when field does not exist on node', () => {
			it('raises an exception', async () => {
				...
				try {
					await adapter.addValueToList(node, 'wizards', 'gandalf');
				} catch (error) {
					expect(error.message).toStrictEqual(
						'Unable to find field for key: wizards'
					);
		...
		describe('when field exists and is an array', () => {
			...
			it('returns the updated node', async () => {
				...
				const result = await adapter.addValueToList(node, 'wizards', 'gandalf');
	...
	describe('removeValueFromList()', () => {
		describe('when field does not exist on node', () => {
			it('raises an exception', async () => {
				...
				try {
					await adapter.removeValueFromList(node, 'wizards', 'gandalf');
				} catch (error) {
					expect(error.message).toStrictEqual(
						'Unable to find field for key: wizards'
					);
		...
		describe('when the list exists and is an array', () => {
			...
			describe('when the specified value does not exist at the indicated position', () => {
				it('raises an exception', async () => {
					...
					try {
						await adapter.removeValueFromList(node, 'wizards', 'gandalf', 0);
					} catch (error) {
						expect(error.message).toStrictEqual(
